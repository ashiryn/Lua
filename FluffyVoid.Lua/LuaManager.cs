...
namespace FluffyVoid.Lua
{
        ...
        where TLuaScript : LuaScript
    {
        ...
        /// <summary>
        ///     List of paths to use for finding required modules that are needed for the
        ///     loaded lua scripts to compile
        /// </summary>
        ...
        /// <summary>
        ///     Predefined empty arguments table that can be reused to ensure arguments are
        ///     always non-null
        /// </summary>
        ...
        /// <summary>
        ///     Event used to notify listeners that a new script has been loaded into
        ///     memory
        /// </summary>
        ...
        ///     Initializes any other Lua dependencies that need to be injected into the
        ///     Lua
        ///     framework
        /// </param>
        ...
        public void LoadScript(string scriptName)
        {
            ...
            if (TryFindLuaFile(RootDirectory, scriptName,
                ...
                TryParseLuaFile(fullFilePath, out script) &&
                script != null)
            {
            ...
            else
            {
                LogManager
                    .LogInfo(
                        $"{scriptName}.lua was not found within {Path.GetFullPath(RootDirectory)} or any of its sub folders",
                        LuaCategory);
            }
        }
        /// <summary>
        ///     Loads and initializes all the Lua files found within the directories into
        ///     Lua Scripts
        /// </summary>
        ...
        /// <summary>
        ///     Clears out the currently loaded lua scripts and reloads all from the
        ///     default directory of the manager
        /// </summary>
        ...
        /// <returns>
        ///     True if the event was successfully called without errors, otherwise
        ///     false
        /// </returns>
        public bool TryCallEvent(string                      scriptName,
                                 string                      eventName,
        ...
        {
            EmptyArguments.Clear();
            return TryCallEvent(scriptName, eventName, EmptyArguments,
                                out output);
        }
        /// <summary>
        ///     Attempts to make a call to an event within the desired Lua script
        ...
        /// <typeparam name="TCustomLua">
        ///     The type of Lua script to attempt to retrieve from
        ///     the manager
        /// </typeparam>
        /// <returns>
        ///     True if the Lua script existed and is of the desired type, otherwise
        ///     false
        /// </returns>
        ...
        public void UnloadScript(string scriptName)
        {
            ...
            else
            {
                LogManager
                    .LogWarning(
                        $"Unable to unload {scriptName}, it is either not loaded or failed to be removed!",
                        LuaCategory);
            }
        }
        /// <summary>
        ...
        /// <summary>
        ///     Allows for additional logic/elements to be added to the arguments table
        ///     prior to dispatching the Lua function call
        /// </summary>
        /// <param name="script">
        ///     The Lua script that we are preparing the arguments table
        ///     for
        /// </param>
        /// <param name="arguments">Arguments table to pass into the event</param>
        ...
        /// <summary>
        ///     Attempts to find a Lua file by digging recursively through the directories
        ///     within the root until the file is found
        ///     or not
        /// </summary>
        ...
        /// <param name="filePath">
        ///     The file path the Lua script was found at if one was
        ///     found
        /// </param>
        /// <returns>True if the Lua script was found, otherwise false</returns>
        protected bool TryFindLuaFile(string     currentDirectory,
                                      string     scriptName,
                                      out string filePath)
        {
            ...
            string libraryModules =
                Path.Combine(currentDirectory, "Libs", "?.lua");
        ...
        /// <summary>
        ///     Attempts to parse the contents of a Lua script file into the Lua script
        ///     class
        /// </summary>
        /// <param name="filePath">The filepath to the Lua script file</param>
        /// <param name="script">The populated Lua script class</param>
        /// <returns>
        ///     True if the Lua script was successfully loaded and populated,
        ///     otherwise false
        /// </returns>
        protected bool TryParseLuaFile(string filePath, out TLuaScript? script)
        {
            ...
            catch (Exception ex)
            {
                LogManager
                    .LogException(
                        $"Unable to create lua script from {Path.GetFullPath(filePath)} path",
                        LuaCategory, ex: ex);
        ...
        /// <summary>
        ///     Recursive helper function used to dig through the directories to load all
        ///     the Lua files within into Lua script
        ///     classes
        /// </summary>
        ...
        private void LoadLuaFiles(string directory)
        {
                                             ...
                                             .Contains("libs")))
            {
                LogManager
                    .LogInfo(
                        $"Loading Lua files from {Path.GetFullPath(subDirectory)}",
                        LuaCategory, Yellow);
