...
{
    /// <summary>
    ///     Base class for all Lua scripts that can be compiled and managed by the Lua
    ///     manager
    /// </summary>
    public abstract class LuaScript
    {
        ...
        /// <summary>
        ///     Application class exposed to the Lua script that holds shared application
        ...
        protected LuaScript(string filePath)
        {
            if (string.IsNullOrEmpty(filePath))
            {
                LogManager
                    .LogError(
                        "Empty or null filepath provided during Lua script creation",
                        LuaCategory);
        ...
        ///     Collection of module paths to load with the Lua
        ///     script
        /// </param>
        ...
        public bool TryCallEvent(string                     eventName,
                                 ...
                                 out DynValue               output)
        {
            if (!TryGetEvent(eventName, out Closure? function) ||
                function == null)
            {
                output = DynValue.Nil;
                return false;
            ...
            catch (ScriptRuntimeException ex)
            {
                ...
                LogManager
                    .LogException(
                        $"Lua run-time exception occurred while calling {eventName} in {Name}.lua. Error occurred in {builder} at Line # {ex.CallStack[0].Location.FromLine}",
                        LuaCategory, ex: ex);
            ...
            catch (Exception ex)
            {
                LogManager
                    .LogException(
                        $"Unhandled Lua run-time exception while calling {eventName} in {Name}.lua.",
                        LuaCategory, ex: ex);
        ...
        /// <summary>
        ///     Attempts to retrieve an event by name that has been registered by the Lua
        ///     script
        /// </summary>
        ...
        /// <summary>
        ///     Helper function used to load the file into the Lua script, handling any
        ///     loading exceptions
        /// </summary>
        ...
        private bool LoadLuaString()
        {
            if (Lua == null)
            {
                LogManager
                    .LogError(
                        "Core Lua class has not been created. Please call LoadScript to create this object.",
                        LuaCategory);
            ...
            catch (ArgumentNullException ex)
            {
                LogManager
                    .LogException(
                        "The path to the Lua script was null or empty, unable to load the Lua script.",
                        LuaCategory, ex: ex);
            ...
            catch (IOException ex)
            {
                LogManager
                    .LogException(
                        $"Unable to read the contents of the Lua file at {FilePath}.",
                        LuaCategory, ex: ex);
            ...
            catch (SyntaxErrorException ex)
            {
                ...
                if (!string.IsNullOrEmpty(ex.DecoratedMessage))
                {
                    ...
                    if (lineInformation is
                        {
                            Success: true, Groups.Count: > 1,
                        })
                    {
                ...
                LogManager
                    .LogException(
                        $"Lua syntax error detected while loading the contents of the Lua file -- {exceptionInformation}.",
                        LuaCategory, ex: ex);
            ...
            catch (Exception ex)
            {
                LogManager
                    .LogException(
                        "Unhandled exception occurred while loading the contents of the Lua file.",
                        LuaCategory, ex: ex);
